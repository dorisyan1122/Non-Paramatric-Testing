...
This KDE estimates the probability density of height-for-age Z score (HAZ) difference in the sample. HAZ difference ranges from -1.49 to 1.24 with a mean and a median of 0.01. The curve is symmetrical and bell-shaped, indicating that the change in HAZ across control and treatment group is normally distributed in the sample. HAZ difference is concentrated at 0, meaning that in the 296 observations (excluding NA values), there is mostly no difference in HAZ pre-intervention and post-intervention. 
...
```
This KDE estimates probability density of height-for-age Z score (HAZ) after intervention. HAZ after intervention ranges from -5.35 to -0.81 with a mean  of -3.40 and a median of -3.38. Similar to KDE of HAZ at baseline, observations are concentrated at around -3, meaning that a HAZ of about -3 after intervention is the most common in the sample. The distribution is less skewed but is still right skewed since the median is less the mean. There are more children with a lower HAZ than a higher HAZ post intervention.
...
# Visualize the relationship between weight-for-age Z score and height-for-age Z score at baseline 
feeding_LOESS|>
...
```
Children's weight-for-age Z score and height-for-age Z score at baseline are positively correlated. It makes sense because the more that the child weighs, the greater the height.

```{r}
# Visualize the relationship between weight-for-age Z score and height-for-age Z score post intervention
feeding_LOESS|>
  ...
  geom_smooth(method = "loess") +
    labs(title = "Relationship between WAZ and HAZ at the Exit of the Study", 
         ...
         y = "Height-for-age Z score (HAZ)")
```
Children's weight-for-age Z score and height-for-age Z score at the exit of the study are positively correlated. It makes sense because the more that the child weighs, the greater the height.
...
3\. The population have equal variances, verified by the test below. There is insufficient evidence to reject the null that the population have equal variances. The variances are equal.
...
In non-parametric testing, there are less assumptions. For example, it only assumes that data from the population is continuous, with no assumption on the specific distribution.
